...
## 更新摘要
**已更新内容**
- 更新了“延迟日志显示与分批加载”章节，详细说明了V2.1.4版本中对UI日志加载逻辑的优化，包括延迟日志显示、分批渲染和系统日志过滤功能的改进。
- 新增了“最大日志行数限制”章节，解释了如何通过设置限制日志显示行数以减轻UI负担。
- 更新了“性能考量”章节，增加了关于最大日志行数限制的性能优化说明。
- 更新了“故障排查指南”，增加了关于日志被跳过的说明。
- 更新了“附录”中的“如何配置日志级别与过滤器”章节，增加了最大日志行数的配置说明。
...
## 简介
本文档面向“看海量化回测系统”的日志系统，聚焦于 GUI 层的日志实现，特别是自定义日志处理器 GUILogHandler 如何将 Python logging 的日志记录转发到 PyQt5 的 GUI 界面；详细解释 log_signal 的使用、_log_message 槽函数如何安全地更新 QTextEdit 中的日志显示；说明多级过滤（INFO、DEBUG、WARNING、ERROR、TRADE）的实现机制与颜色编码样式；阐述延迟日志显示与批量更新策略（delayed_logs 与 display_delayed_logs 的分批加载）；并提供在不同模块中记录日志的方法与最佳实践。
...
### 最大日志行数限制
- 在设置界面中，用户可以配置最大日志显示行数，默认值为1000，范围为100-100000。
- 当延迟日志的数量超过最大行数时，系统会自动跳过最早的日志，仅显示最新的日志，以减轻UI负担。
- 该功能通过 SettingsDialog.py 中的 max_log_lines_edit 控件实现，并在 GUIkhQuant.py 的 display_delayed_logs 方法中应用。

**章节来源**
- [SettingsDialog.py](file://SettingsDialog.py#L172-L204)
- [GUIkhQuant.py](file://GUIkhQuant.py#L4595-L4601)
...
- **最大日志行数限制**：当延迟日志数量超过用户设置的最大行数时，系统会自动跳过最早的日志，仅显示最新的日志，有效减轻UI负担，防止高频率日志导致界面假死。
...
- 日志被跳过：
  - 检查日志消息是否包含系统日志关键字，如“加载进度”、“检查更新”等，这些日志默认被过滤。
  - 检查延迟日志数量是否超过最大行数限制，系统会自动跳过最早的日志。
...
- **最大日志行数**：
  - 在设置界面中，通过“最大日志显示行数”输入框配置，范围为100-100000，默认值为1000。
  - 该设置在 GUIkhQuant.py 的 display_delayed_logs 方法中应用，当延迟日志数量超过最大行数时，系统会自动跳过最早的日志，仅显示最新的日志。
