...
## 更新摘要
**变更内容**
- 在“详细组件分析”部分新增了关于T+1交易模式下新交易日开始时持仓可用数量更新机制的说明
- 更新了“回测主循环 run_backtest_loop”流程图，以反映新的持仓更新逻辑
- 在“投资组合状态更新逻辑”部分补充了T+1模式下持仓可用数量的更新时机
...
- 触发器与数据周期一致性：根据触发器类型与数据周期一致性进行检查，避免不匹配导致的误触发或漏触发。
